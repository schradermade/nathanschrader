...
`}
  ...
  useEffect(() => {
    ...
    const allSectionIds = Object.values(projectSections)
      .flat()
      .map((section) => section.id);
    const projectIds = Object.values(projectViews)
      .flat()
      .map((section) => section.id);
    const mergedIds = [...allSectionIds, ...projectIds];
    if (hash && mergedIds.includes(hash)) {
      const viewOwner = Object.entries(projectViews).find(([, list]) =>
        list.some((section) => section.id === hash)
      );
      if (viewOwner) {
        setActiveProject(viewOwner[0]);
      }
      const owner = Object.entries(projectSections).find(([, list]) =>
        list.some((section) => section.id === hash)
      );
      if (owner) {
        setActiveProject(owner[0]);
      }
      setActiveId(hash);
    }
  }, []);
  ...
  const handleProjectSelect = (projectId: string) => {
    ...
    if (firstSection) {
      ...
      window.history.replaceState(null, '', `#${firstSection.id}`);
    }
  };
      ...
      <aside className="nav single-nav" ref={navRef}>
        ...
        <div className="nav-group">
          <button
            ...
            onClick={() => handleProjectSelect('main')}
          >
            About
          </button>
        </div>
